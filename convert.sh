...
$FONTFORGE_BIN -lang=ff -c 'Open($1); Generate($2)' sources/Vertograd.sfd sources/Vertograd-Regular.ufo

# Dirty hack to add the openTypeOS2Selection key to the fontinfo.plist file
FONTINFO=`cat sources/Vertograd-Regular.ufo/fontinfo.plist`
echo "${FONTINFO/    <key>openTypeOS2Type<\/key>/    <key>openTypeOS2Selection</key>$'\n'    <array>$'\n'      <integer>7</integer>$'\n'    </array>$'\n'    <key>openTypeOS2Type</key>}" > sources/Vertograd-Regular.ufo/fontinfo.plist
